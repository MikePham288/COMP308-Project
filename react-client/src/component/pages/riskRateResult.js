...
const RiskRateResults = (props) => {
  ...
  const getPatientCategory = async () => {
    axios
      .post(trainUrl, location.state.data)
      .then((result) => {
        setLoading(false);
  ...
  return (
    <div className="card-container">
      <div className="row justify-content-center">
        <h2>
          Patient's <span className="text-danger">Heart Attack Risk Rate</span>
        </h2>
      </div>
      ...
      <div className="row justify-content-center row-padding">
        <button className="btn btn-primary" onClick={backToDetails}>
          Go Back To Details
        </button>
      </div>
    </div>
...
export default RiskRateResults;
