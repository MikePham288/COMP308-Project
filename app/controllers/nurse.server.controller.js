...
exports.getAllPatientsByNurseId = async (req, res) => {
  ...
  Patient.find({ nurse: req.user.id }, (err, patients) => {
    if (err) {
      res
        .status(500)
        .send({ msg: "Error Fetching Patients for Nurses", err: err });
    } else {
      ...
      if (tip) {
        ...
        Patient.findByIdAndUpdate(req.patient._id, pat, (err, patient) => {
          if (err) {
            ...
            res
              .status(200)
              .send({ msg: "successfully updated", patient: patient });
          }
        });
      } else {
        res
          .status(500)
          .send({ msg: "server error motivational tip not created" })
          .end();
      }
    }
  });
      ...
      if (sign) {
        ...
        Patient.findByIdAndUpdate(req.patient._id, pat, (err, patient) => {
          if (err) {
            ...
            res
              .status(200)
              .send({ msg: "succesfully updated", patient: patient });
          }
        });
      } else {
        res
          .status(500)
          .send({ msg: "server error vital signs not created" })
          .end();
      }
    }
  });
  ...
  await clinicalData.save((err, data) => {
    if (err) {
      ...
      if (data) {
        ...
        res
          .status(500)
          .send({ msg: "server error clinical data not creates" })
          .end();
      }
    }
  });
